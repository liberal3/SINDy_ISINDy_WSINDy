...
ode_num = 5;                        % select ODE from list below
tol_ode = 1e-12;                    % set tolerance (abs and rel) of ode45
noise_ratio = 0.05;                  % set ||noise||_2/||clean signal||_2. If negative, sets sigma directly to -noise_ratio
...
% phi_class =1;                                  % ---1 = piecewisepoly test functions, 
                                    % ---2 = gaussian test functions
                                    % ---function_handle = use function
                                    % ---handle and symbolically compute
                                    % ---derivative
% tau = 10^-16; tauhat = -1;        % DEFAULT: [tau,tauhat] = [10^-16,-1]. 为r_whm
tau = 10^-16; tauhat = 30;          % ---tau > 1: [tau,tauhat] = [m,p] directly. else...
                                    % ---tauhat > 0: tauhat = width-at-half-max. tauhat < 0: use corner point (recommended)
K_frac = 126;                       % DEFAULT: K_frac = 1.
